...
% Để hiện thực hóa mục tiêu này, các nhà phát triển có thể áp dụng phối hợp nhiều giải pháp kỹ thuật cụ thể. Một số cơ chế bảo mật tiêu biểu có thể kể đến như: giảm thiểu bề mặt tấn công; thiết lập nghiêm ngặt phân quyền admin-user theo nguyên tắc đặc quyền tối thiểu; sử dụng SELinux để kiểm soát truy cập bắt buộc; áp dụng dm-verity nhằm đảm bảo tính toàn vẹn của hệ điều hành và dùng fscrypt để mã hóa bảo mật dữ liệu người dùng.
% \begin{figure}[!h]
%     \centering
...
\subsection{Giảm thiểu tấn công bề mặt}

Giảm thiểu tấn công bề mặt (Attack Surface Reduction) là phương pháp hạn chế số lượng thành phần, dịch vụ và chức năng có thể bị khai thác trong hệ thống. 
Đối với Linux nhúng, bề mặt tấn công thường bao gồm các dịch vụ mạng không cần thiết, các package dư thừa hoặc các cấu hình mặc định thiếu an toàn.

Yocto Project cho phép xây dựng image Linux tối giản và chỉ tích hợp các thành phần cần thiết cho hệ thống. 
Điều này giúp giảm số lượng tiến trình đang hoạt động, hạn chế cổng mạng mở và giảm nguy cơ xuất hiện lỗ hổng bảo mật.

Trong đề tài này, nguyên tắc giảm thiểu bề mặt tấn công được sử dụng làm nền tảng cho việc xây dựng image Linux nhúng tối ưu và an toàn hơn cho gateway IoT.

\subsection{Bảo mật quyền truy cập với SELinux}

SELinux (Security-Enhanced Linux) là cơ chế bảo mật dành cho Linux dựa trên mô hình Mandatory Access Control (MAC). 
Khác với mô hình phân quyền truyền thống, SELinux cho phép kiểm soát chặt chẽ quyền truy cập giữa các tiến trình và tài nguyên hệ thống thông qua các policy bảo mật.

SELinux hoạt động bằng cách gán security context cho tiến trình và tài nguyên trong hệ thống. 
Mọi hành động truy cập đều phải tuân theo policy đã được định nghĩa trước; nếu không được cho phép rõ ràng, truy cập sẽ bị từ chối theo mặc định.

Một trong những đặc điểm quan trọng của SELinux là nguyên tắc đặc quyền tối thiểu (least privilege), theo đó mỗi tiến trình chỉ được cấp các quyền cần thiết cho chức năng của mình. 
Điều này giúp hạn chế khả năng lan rộng của tấn công khi một dịch vụ bị khai thác.

Trong các hệ thống Yocto Linux, SELinux thường được tích hợp thông qua layer \texttt{meta-selinux} nhằm tăng cường khả năng bảo vệ cho hệ điều hành nhúng.
\begin{figure}[!h]
    \centering
