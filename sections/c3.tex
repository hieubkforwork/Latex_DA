\section{Tổng quan về Linux nhúng và Yocto Project}
...
%     \label{fig:tenlabel}
% \end{figure}
% Trong đề tài này, Edge AI được triển khai như một module phân tích dữ liệu chuỗi thời gian (Time Series), đảm nhiệm chức năng dự báo sớm nồng độ bụi mịn PM2.5 dựa trên các thông số môi trường thực tế ngay tại tại hiện trường.
...
% \begin{figure}[H]
%     \centering
%     \includegraphics[width=0.6\textwidth]{image/TCN.png} 
%     \caption{Kiến trúc Model TCN}
%     \label{fig:tenlabel}
% \end{figure}
...
% Sau quá trình thực nghiệm và tối ưu hóa cho hệ thống Edge AI, kiến trúc TCN được lựa chọn thay thế cho các mô hình LSTM và RNN nhờ những ưu điểm sau:

% \begin{itemize}
%     \item \textbf{Khả năng xử lý song song:}  
%     Không giống như RNN và LSTM phải tính toán tuần tự theo từng bước thời gian, TCN sử dụng phép tích chập trên toàn bộ chuỗi dữ liệu. Điều này cho phép tận dụng khả năng tính toán song song của phần cứng, từ đó rút ngắn đáng kể thời gian huấn luyện và suy luận.

%     \item \textbf{Giảm hiện tượng mất mát đạo hàm:}  
%     Nhờ cấu trúc tích chập phân tầng kết hợp với các kết nối tắt (\textit{residual connections}), TCN có khả năng học ổn định hơn khi xử lý các phụ thuộc dài hạn so với RNN truyền thống.

%     \item \textbf{Trường thụ cảm linh hoạt:}  
%     Bằng cách điều chỉnh các tham số như \texttt{kernel\_size} và \texttt{dilations}, có thể kiểm soát chính xác độ dài của chuỗi dữ liệu quá khứ mà mô hình sử dụng để đưa ra dự báo.

%     \item \textbf{Phù hợp với Edge AI:}  
%     Kiến trúc TCN có cấu trúc gọn nhẹ, số lượng tham số thấp và dễ dàng chuyển đổi sang định dạng TensorFlow Lite Float16. Trong đề tài này, mô hình sau khi lượng tử hóa đã giảm khoảng 50\% dung lượng lưu trữ trong khi vẫn duy trì độ chính xác dự báo ở mức cao, phù hợp để triển khai trên thiết bị nhúng như Raspberry Pi 4.
% \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Hiệu quả trong dự báo PM2.5:}
    Nhiều nghiên cứu gần đây cho thấy các biến thể của TCN đạt kết quả rất tốt trong bài toán dự báo chất lượng không khí và nồng độ PM2.5 \cite{li2023_tcn_bilstm_dmattention,wang2024_causal_stan,stoten2025_pm25}.
